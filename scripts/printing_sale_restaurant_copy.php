...
*/
$sql_invoice = 'select * from invoice_sale_restaurant_2 where `invoice_sale_id`="'.$_SESSION['invoice_no'].'"';
$invoice=mysqli_fetch_assoc(execute_query($sql_invoice));

$sql1 = 'select * from stock_sale_restaurant_2 where invoice_no="'.$_SESSION['invoice_no'].'"';
		...
		<div id="wrapper">
			...
			<div id="address" style="margin-top:10px">
				<p style="font-size:15px;letter-spacing: 1px"><?php
						if($gstin!=''){
							echo "<br/><strong style='font-size:15px;letter-spacing: 1px'>GSTIN : 09AAHCS2262A1ZN</strong>";
						}
					?> </p>
			</div>
			<div id="address" style="margin-top:5px">
			...
			<div id="invoice">
				<div id="bill_detail">
					<table border="0" bordercolor="#ccc" cellpadding="0" cellspacing="0" width="100%" > 
					  ...
					  <td style="font-size: 15px">Invoice No : <?php echo $invoice['invoice_no']; ?>
							</td>
					   </tr	>
						<tr style="height:20px;font-size:15px">
				...
				<table border="0" bordercolor="#ccc"  cellpadding="0"  cellspacing="0" width="100%">
					<div id="bill">
						...
						while($row=mysqli_fetch_assoc($res)) {
							...
							<td id="align_right" style="font-size: 12px"><?php 
							...
							<td id="align_right" style="border-right:2px solid;font-size: 13px"><?php echo $row['amount'];?></td>
							</tr>
							<?php 
							if(preg_match("/%/", $row['discount'])){
								$e_price = $row['basicprice']-($row['basicprice']*($row['discount']/100));
						...
						<tr style="height:18px">
							<td colspan="2" style="text-align:right; padding-right:10px; border:1px solid;font-size:14px"><b style="font-size:12px">Total</b></td>
						...
						<tr>
							<td colspan="4" style="text-align:center; padding-right:10px; border-top:1px solid;border-left:1px solid;font-size:13px"><b style="font-size:12px">CGST : 2.5%</b></td>
						...
						<tr style="height:20px">
							...
							<td style="text-align:right; border:1px solid;font-size:12px"><?php echo round($invoice['grand_total'] , 2);?></td>
						</tr>
						<tr>
							<td colspan="<?php echo $total_col; ?>" style="border:0px solid;font-size:14px">&nbsp;</td>
						</tr>
						<tr>
							<td colspan="<?php echo $total_col; ?>" style="font-size:15px">In Words.: <?php 
							$tot_amount = round(($invoice['grand_total']),0);
							echo strtoupper(int_to_words($tot_amount)); ?> </td>
						</tr>
						<?php
						if($firm_type=='non_composition'){
							$sql = 'select part_id, stock_sale_restaurant_2.vat as vat, vat_value as vat_value, stock_sale_restaurant_2.excise as excise, excise_value as excise_value, taxable_amount as taxable_amount, part_no, amount from stock_sale_restaurant_2 join stock_available on stock_available.sno = part_id where invoice_no='.$_SESSION['invoice_no'].' order by s_no';
							//echo $sql;
							echo '<tr>
						...
						<tr>
							<td colspan="<?php echo $total_col; ?>" style="border:none;">
								<div style="width: 100%; border: 0px solid; float: left;"><br>
									<span style="font-size:12px">Registration No. 02/476 J.A./Sarai Act/2003. CIN: U55101UP1980PTC005050</span><br><br>
									<b><u>Terms &amp; Condition</u></b>
									<p id="terms"><?php echo $terms; ?></p>
								</div>
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<div id="wrapper">
			<input type="button" id="btnPrint" onclick="window.print();" value="Print Page" />
			<input type="button" id="btnPrint" onclick="location.href='../sale_new.php?id=<?php echo $_SESSION['invoice_no']; ?>';" value="Combine Print" />
			<input type="button" id="btnPrint" onclick="location.href='../report_sale.php?del=<?php echo $_SESSION['invoice_no']; ?>'; return confirm('Are you sure?')" value="Delete" />
			<div id="company_info">
				<center><p style="font-size:14px; text-decoration: underline;"><?php if($invoice['invoice_type']=='SALE'){echo "INVOICE";}elseif($invoice['invoice_type']=='TAX'){echo "INVOICE";}else{echo "";}?></p></center>
				<div id="company">
			...
			<table width="100%">
				<tr>
					<td colspan="5">
						...
						<?php } else{?>
						<center><p style="font-size:24px;"><?php echo "TAX INVOICE ";?></p></center>
						<?php } ?>
					</td>
				</tr>
				<tr>
					<th align="left" width="20%">Invoice No.</th>
					<th align="left"><?php echo $invoice['invoice_no']; ?></th>
				</tr>
				<tr>
					<th align="left">Guest Name : </th>
					<th align="left"><?php echo $cust['cust_name']; ?></th>
					<th >&nbsp;</th>
			...
			<div id="invoice">
				<table border="0" bordercolor="#ccc"  cellpadding="0"  cellspacing="0" width="100%">
					<div id="bill">
						...
						<?php
						if($firm_type=='non_composition'){
							$sql = 'select part_id, stock_sale_restaurant_2.vat as vat, vat_value as vat_value, stock_sale_restaurant.excise as excise, excise_value as excise_value, taxable_amount as taxable_amount, part_no, amount from stock_sale_restaurant_2 join stock_available on stock_available.sno = part_id where invoice_no='.$_SESSION['invoice_no'].' order by s_no';
							//echo $sql;
							echo '<tr>
