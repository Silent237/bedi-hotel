...
$state = mysqli_fetch_assoc(execute_query($sql));
	...
	$_POST['allot_from'] = $_SESSION['allot_from'];
	$_POST['allot_to'] = $_SESSION['allot_to'];
	 ...
	 $sql_receipt_type=$_SESSION['sql_receipt_type'];
       ...
       $html ='<table border="1">
            ...
            <tr>
				...
				<th width="21%" '.$rowspan.'>Invoice Date</th>
				<th width="21%" '.$rowspan.'>Room No.Guest Name</th>
				<th width="21%" '.$rowspan.'>GST No.</th>
			...
			while($row = mysqli_fetch_array($result)){
				...
				if(abs($state['rate'])==abs($row['state'])){
						$html .= '<td class="number">'.number_format((float)($tax*$days), 2, '.', '').'</td>
