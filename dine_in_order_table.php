...
<?php
	...
	function insert_data(id){
		...
		totalPrice = parseFloat($("#show_total").html()) || 0;
		...
		var flag = chk_row ? 1 : 0;

		if(flag == 1) {
			let unit = parseFloat($("#unit_input_"+id).val());
			unit++;
			$("#unit_input_"+id).val(unit);
			update_quantity(id);
		} else {
			let unit = 1;
			let unit1 = 1;
			...
			totalPrice += price;
			$("#show_total").html(totalPrice.toFixed(2));
			$("#show_total1").html(totalPrice.toFixed(2));
			$('#mytable > tbody:last').append(
				'<tr id="row_' + id + '">' +
				'<td><input type="number" min="1" value="' + unit + '" id="unit_input_' + id + '" name="unit_data_' + id + '" onchange="update_quantity(' + id + ');" style="width:50px;"></td>' +
				'<input type="hidden" name="unit1_' + id + '" id="unit1_' + id + '" value="' + unit1 + '">' +
				...
				'<td id="item_price' + id + '">' + price.toFixed(2) + '</td>' +
				'<input type="hidden" name="item_price' + id + '" id="price1_' + id + '" value="' + price.toFixed(2) + '">' +
				'<input type="hidden" name="unit_price_' + id + '" id="unit_price_' + id + '" value="' + price.toFixed(2) + '">' +
				'<input type="hidden" name="data_id_' + current_id_other + '" id="data_id_' + current_id_other + '" value="' + id + '">' +
				'<td><button type="button" onclick="remove_row(' + id + ')"><i class="fas fa-close" style="color:red;"></i></button></td>' +
				'</tr>'
			);
		}
	...
	function update_quantity(id) {
		let qty = parseFloat($('#unit_input_' + id).val()) || 1;
		let unit_price = parseFloat($('#unit_price_' + id).val()) || 0;
		let total = qty * unit_price;
		$('#price1_' + id).val(total.toFixed(2));
		$('#item_price' + id).html(total.toFixed(2));

		let grand_total = 0;
		$('input[id^="price1_"]').each(function () {
			let val = parseFloat($(this).val());
			if (!isNaN(val)) grand_total += val;
		});
		$('#show_total').html(grand_total.toFixed(2));
		$('#show_total1').html(grand_total.toFixed(2));
		update_total_quantity();
	}
	...
	function update_total_quantity() {
		...
		$("input[id^='unit_input_']").each(function () {
			let val = parseFloat($(this).val());
			if (!isNaN(val)) total_quantity += val;
		});
		$("#total_quantity").val(total_quantity);
		if (total_quantity < 1) $('#done').hide(); else $('#done').show();
	}
