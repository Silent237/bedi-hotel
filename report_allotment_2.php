...
if(isset($_POST['einv_form'])){
	...
	$_SESSION['allot_from'] = $_POST['allot_from'];
	$_SESSION['allot_to'] = $_POST['allot_to'];
	...
	while($row = mysqli_fetch_assoc($result)){
		...
		if(isset($_POST['einv_'.$row['sno']])){
			...
			$BuyerDtls = array("Gstin"=>$row['id_2'], "LglNm"=>$row['company_name'], "Addr1"=>($row['address'] != '')?$row['address']:$details['guest_address'], "Addr2"=>$row['city'], "Loc"=>$row['city'], "Pos"=>$state['rate'], "Pin"=>$pincode, "Stcd"=>$row['state']);
...
$("input#cust_name1").on("keydown.autocomplete", function() {
 ...
 <div id="container">
	...
	<form action="" class="wufoo leftLabel page1" id="report_allotment" name="addnewdesignation" enctype="multipart/form-data" method="post" onSubmit="" >
		...
		<table>
			<tr  class="no-print">
				<td colspan="20" float="left" class="text-center">
				<a href="room_bill_summery_export_2.php"><input type="button" style=" background-color: #f44f4f; color:white;width:200px" name="student_ledger" class="form-control btn btn-danger"  style="float: left;" value="Download In Excel"></a></span>
				</td>
			</tr>
			<tr>
				<th width="5%" <?php echo $rowspan; ?>>S.No.</th>
				<th width="21%" <?php echo $rowspan; ?>>Invoice Date</th>
				<th width="21%" <?php echo $rowspan; ?>>Room No.Guest Name</th>
				<th width="21%" <?php echo $rowspan; ?>>GST No.</th>
			...
			while($row = mysqli_fetch_array($result)){
				...
				if($row['cancel_date']==''){
					if($row['invoice_type']=='tax'){
						$tot_tax_discount+=floatval($row['other_discount'])*$days;
						...
						$tot_discount+=floatval($row['other_discount'])*$days;
					...
					$grand_tot_discount+=floatval($row['other_discount'])*$days;
				...
				echo '<tr '.$row_col.'>
				<td>'.$i++.'</td>
				<td>'.date("d-m-Y", strtotime($row['bill_create_date'])).'</td>
				<td><b>'.$row['room_name'].'</b><br><b>Company Name:</b>'.$row['company_name'].'<br /><b>Guest Name:</b>'.$row['guest_name'].'<br />In : '.$row['allotment_date'].'<br />Out :'.$row['exit_date'].'<br />Days : '.$days.$cancel.'</td>
				<td>'.$gstin.'</td>
				...
				$row['other_discount']=floatval($row['other_discount'])+floatval($row['discount_value']);
				if($pos===false){
